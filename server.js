...
async function translateSingleChunk(text, sourceLanguage, targetLanguage, retryCount = 0) {
  ...
  if (primaryService === 'googletrans' || primaryService === 'auto') {
    try {
      ...
      console.log(`⚠️ Python API failed, using MyMemory as 2nd fallback...`);
      try {
        const memResponse = await axios.get('https://api.mymemory.translated.net/get', {
          params: {
            q: text,
            langpair: `${sourceLanguage}|${targetLanguage}`,
            de: 'gcbdevs@gmail.com',
            key: 'f1f6b192448413778691'
          },
          timeout: 15000
        });
        if (memResponse.data.responseStatus === 200) {
          ...
          if (translated && translated !== text) {
            console.log(`✅ MyMemory (2nd fallback): ${text.length} → ${translated.length} chars`);
            return translated;
          }
        }
      } catch (memError) {
        console.log(`❌ MyMemory (2nd fallback) failed: ${memError.message}`);
      }
    }
  }
  ...
  if (isLongText) {
    ...
    try {
      ...
      // ALWAYS try Google Translate as fallback when MyMemory fails
      ...
      try {
        ...
        // If Google also fails, try LibreTranslate
        console.log(`⚠️ Trying LibreTranslate as second fallback...`);
        await new Promise(resolve => setTimeout(resolve, 300));
...
// Detect if text is already in French
...
// Detect if text is already in target language (English)
function isLikelyEnglish(text) {
  if (typeof text !== 'string') return false;
  
  const englishIndicators = ['the', 'and', 'for', 'with', 'from', 'weight', 'range', 'degrees', 'thread', 'mount'];
  const lowerText = text.toLowerCase();
  
  // Check for English words
  if (englishIndicators.some(word => lowerText.includes(word))) {
    return true;
  }
  
  // Check for common English patterns
  if (lowerText.includes('drag') || lowerText.includes('fluid') || lowerText.includes('system') ||
      lowerText.includes('balance') || lowerText.includes('technology') || lowerText.includes('attach')) {
    return true;
  }
    ...
    if (translationResponse && translationResponse.data) {
      ...
      if (translations.length > 0) {
        ...
        for (const searchKey of possibleKeys) {
          metafieldTranslation = translations.find(t => t.key === searchKey);
          if (metafieldTranslation) {
            console.log(`✅ Found translation with key: ${searchKey}`);
            break;
          }
        }
        ...
        if (!metafieldTranslation) {
          ...
          for (const t of translations) {
            if (t.value && (t.value.trim().startsWith('{') || t.value.trim().startsWith('['))) {
              ...
              console.log(`✅ Found JSON translation with key: ${t.key}`);
              break;
            }
          }
        }
        ...
        if (metafieldTranslation) {
          ...
          console.log(`⚠️ No metafield translation found. Full translation list:`, translations);
        }
      } else {
        console.log(`❌ No translations returned from API`);
        console.log(`Full response:`, JSON.stringify(translationResponse.data, null, 2));
        if (translationResponse.data.errors) {
          console.log(`❌ GraphQL Errors:`, translationResponse.data.errors);
      }
      }
